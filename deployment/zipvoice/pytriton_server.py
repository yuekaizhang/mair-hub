...
class ZipVoiceModel:
    def __init__(self, model_dir, model_name, trt_engine_path, reference_audio_sample_rate, device="cuda"):
        ...
        if self.reference_sample_rate != self.sampling_rate:
            self.resampler = torchaudio.transforms.Resample(self.reference_sample_rate, self.sampling_rate)# .to(
                # self.device
            # )
        else:
            self.resampler = None
